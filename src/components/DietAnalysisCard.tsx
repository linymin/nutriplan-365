...
export const DietAnalysisCard = ({ weeklyPlan: propPlan, previousWeekNutrition }: DietAnalysisCardProps) => {
  const { weeklyMealPlan } = useMealPlan();
  const weeklyPlan = propPlan || weeklyMealPlan;
    ...
    return (
      <Card className="glass-card">
        <CardHeader>
          ...
          <CardDescription>暂无数据，请先生成本周食谱</CardDescription>
        </CardHeader>
      </Card>
    );
  }
  ...
  const proteinRatio = totalMacros > 0 ? (avgProtein * 4) / totalMacros : 0;
  const carbsRatio = totalMacros > 0 ? (avgCarbs * 4) / totalMacros : 0;
  const fatRatio = totalMacros > 0 ? (avgFat * 9) / totalMacros : 0;
  ...
  return (
    <Card className="glass-card">
      <CardHeader>
        ...
        <CardDescription>本周饮食结构与营养情况</CardDescription>
      </CardHeader>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-4 gap-3">
            ...
            <div className="text-center p-3 rounded-xl bg-blue-500/10">
              ...
              <TrendIcon trend={getTrend(avgProtein, previousWeekNutrition?.protein ? previousWeekNutrition.protein / 7 : undefined)} />
            </div>
            <div className="text-center p-3 rounded-xl bg-amber-500/10">
              <p className="text-xs text-muted-foreground mb-1">碳水</p>
              <p className="font-semibold text-amber-600">{avgCarbs}g</p>
              <TrendIcon trend={getTrend(avgCarbs, previousWeekNutrition?.carbs ? previousWeekNutrition.carbs / 7 : undefined)} />
            </div>
            <div className="text-center p-3 rounded-xl bg-rose-500/10">
              ...
              <TrendIcon trend={getTrend(avgFat, previousWeekNutrition?.fat ? previousWeekNutrition.fat / 7 : undefined)} />
            </div>
          </div>
        </div>
